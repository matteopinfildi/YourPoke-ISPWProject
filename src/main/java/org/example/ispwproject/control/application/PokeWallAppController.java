    ...
    public void registerObserver(PokeWallObserver observer, int sessionId) {
        try {
            ...
            if (observer instanceof PokeWallController pokeWallController) {
                ...
                // Rimuovi observer da tutti i post
                for (PokeWall post : allPosts) {
                    post.removeObserver(observer);
                }

                // Registra observer solo per post non visti
                for (PokeWall post : unseenPosts) {
                    try {
                        ...
                        System.err.println("Errore durante la registrazione per post " +
                                post.getId() + ": " + e.getMessage());
                    }
                }
            }
        } catch (SystemException e) {
            System.err.println("Errore registrazione observer: " + e.getMessage());
        }
    }
